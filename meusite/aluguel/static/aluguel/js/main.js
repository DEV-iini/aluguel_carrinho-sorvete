...
function abrirModalContrato() {
    ...
    const scrollContrato = document.getElementById('corpo-contrato-scroll');
    const aceite = document.getElementById('aceite-contrato');
    const boxAceite = document.querySelector('.contrato-aceite');
    ...
    if (!scrollContrato || !aceite || !btnEnviar) return;

    scrollContrato.scrollTop = 0;
    aceite.checked = false;
    aceite.disabled = true;
    btnEnviar.disabled = true;

    if (boxAceite) boxAceite.classList.add('bloqueado');

    scrollContrato.onscroll = function () {
        const chegouAoFim =
            scrollContrato.scrollTop + scrollContrato.clientHeight >= scrollContrato.scrollHeight - 8;

        if (chegouAoFim) {
            aceite.disabled = false;

            if (boxAceite) {
                boxAceite.classList.remove('bloqueado');
            }
        }
    };
}
...
function alternarBotaoFinalizar() {
    const check = document.getElementById('aceite-contrato');
    const btn = document.getElementById('btn-enviar-reserva');

    if (!check || !btn) return;

    btn.disabled = check.disabled || !check.checked;
}
