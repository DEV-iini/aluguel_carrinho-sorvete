...
function finalizarPedido() {
    ...
    if (selecionados.length === 0) {
        alert("Escolha pelo menos um sabor para continuar.");
        return;
    }
    ...
    document.getElementById('ct-dados-cliente').innerHTML = `
        <strong>Nome/Razao Social:</strong> ${nome}<br>
    ...
    const corpoContrato = document.getElementById('corpo-contrato-scroll');
    if (corpoContrato) corpoContrato.scrollTop = 0;
}
...
function abrirModal() {
    ...
    if (!dataSelecionada) {
        alert("Por favor, selecione uma data no calendario antes de finalizar a reserva.");
        return;
    }
...
function fecharModalContrato() {
    ...
    if (modalReserva && modalReserva.style.display === 'block') {
        document.body.style.overflow = 'hidden';
    } else {
        document.body.style.overflow = 'auto';
    }
...
async function carregarDisponibilidade(mesAno) {
    try {
        ...
        totalCarrinhosGlobal = Number(data.total_carrinhos || 0);
        ocupacaoGlobal = data.ocupacao || {};
    } catch (e) {
        console.error("Aviso: falha de comunicacao com o servidor.", e);
...
function definirClasseOcupacao(dataISO) {
    const reservas = Number(ocupacaoGlobal[dataISO] || 0);
    const total = Number(totalCarrinhosGlobal || 0);

    if (total <= 0) return 'esgotado';

    const percentual = (reservas / total) * 100;
...
function renderizarCalendario(mesAno) {
    ...
    diasSemana.forEach(dia => {
        ...
        div.classList.add('dia-semana-calendario');
        div.innerText = dia;
    ...
    for (let i = 0; i < primeiroDia; i++) {
        ...
        divVazia.classList.add('dia-vazio-calendario');
...
function atualizarSeletorCarrinhos(dataISO) {
    ...
    if (!secao || !select || !aviso) return;

    const ocupados = Number(ocupacaoGlobal[dataISO] || 0);
    const disponiveisHoje = Number(totalCarrinhosGlobal || 0) - ocupados;

    if (disponiveisHoje <= 0) {
        secao.style.display = 'none';
        alert("Desculpe, nao ha carrinhos disponiveis para este dia.");
    ...
    if (disponiveisHoje === 1) {
        aviso.innerText = '1 carrinho disponivel para esta data';
    } else {
        aviso.innerText = `${disponiveisHoje} carrinhos disponiveis para esta data`;
    }
...
function atualizarLabelMesAno(ano, mes) {
    ...
    const label = document.getElementById('mes-ano-display');
    if (label) label.innerText = `${nomesMeses[mes - 1]} ${ano}`;
    ...
    btnAnterior.disabled = ano === hoje.getFullYear() && mes === hoje.getMonth() + 1;
}
